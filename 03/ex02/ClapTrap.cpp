...
ClapTrap::ClapTrap()
	: name("DefaultClap"), hitPts(10), energyPts(10), attDmg(0) {
	std::cout << "ClapTrap " << name << " default constructor called"
			  << std::endl;
}

ClapTrap::ClapTrap(std::string n)
	: name(n), hitPts(10), energyPts(10), attDmg(0) {
	if (n.empty())
...
ClapTrap::~ClapTrap() {
	std::cout << "Clap" << name << " destroyed" << std::endl;
}
...
void ClapTrap::takeDamage(unsigned int amount) {
	hitPts -= amount;
	std::cout << name << " takes " << amount << " damage, " << hitPts
